...
QVariant AdBlockModel::data(const QModelIndex &index, int role) const
{
    ...
    if (!index.isValid() || index.row() >= rowCount()) {
       qDebug() << "!index valid or row count too high" << !index.isValid() << " - " << index.row() << ">=" << rowCount();
        return QVariant();
    }
    ...
    {
        qDebug() << "index.column() == 0 && role == Qt::CheckStateRole";
        return (sub->isEnabled() ? Qt::Checked : Qt::Unchecked);
    }

    if(role != Qt::DisplayRole && role != Qt::EditRole) {

        qDebug() << "role != Qt::DisplayRole && role != Qt::EditRole";
        return QVariant();
    }
