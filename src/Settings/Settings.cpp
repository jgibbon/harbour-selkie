...
{
    QString keyString = m_settingMap.value(key, QLatin1String("unknown"));
...
void Settings::findKeys()
{
    for(auto e : m_settingMap.keys())
    {
        emit keyFound(m_settingMap.value(e), m_settings.value(m_settingMap.value(e)).toString());
    }
}
